...
# Tính tổng lưu lượng của từng máy để xác định máy quan trọng nhất
total_flow = {}
for machine in machine_counts:
    flow_sum = 0
    for (m1, m2), flow in flow_dict.items():
        if m1 == machine:
            flow_sum += flow
        if m2 == machine:
            flow_sum += flow
    total_flow[machine] = flow_sum

# Sắp xếp các máy theo tổng lưu lượng giảm dần
sorted_machines = sorted(total_flow.items(), key=lambda x: x[1], reverse=True)
most_important_machine = sorted_machines[0][0]  # Máy có tổng lưu lượng lớn nhất (ví dụ: MC)

# Bố trí máy
machine_positions = {}  # Lưu tọa độ tâm của máy (theo mét)
occupied_regions = []  # Lưu các vùng đã chiếm (theo mét)
placed_machines = set()
remaining_machines = set()

# Tạo danh sách tất cả máy (bao gồm các bản sao như MAL_1, MAL_2, ...)
...
# Đặt máy quan trọng nhất trước (ở giữa mặt bằng)
for i in range(machine_counts[most_important_machine]):
    machine_name = f"{most_important_machine}_{i}" if i > 0 else most_important_machine
    wid, length = machine_sizes[most_important_machine]
    
    # Chuyển kích thước sang đơn vị ô (1m = 10 ô)
    wid_cells = int(wid * 10)
    length_cells = int(length * 10)

    # Đặt ở vị trí trung tâm (theo mét)
    x = 20 - wid / 2  # Đặt giữa chiều rộng
    y = 10 - length / 2  # Đặt giữa chiều dài
    x_cells = int(x * 10)  # Chuyển sang đơn vị ô
    y_cells = int(y * 10)
    ...
    if not overlap:
        ...
        if region.size == wid_cells * length_cells and np.all(region == None):
            ...
            occupied_regions.append((machine_name, x, y, x + wid, y + length))
            placed_machines.add(machine_name)
            remaining_machines.remove(machine_name)
        else:
            print(f"Không đủ diện tích để bố trí máy {machine_name} ở vị trí trung tâm!")
    else:
        print(f"Không thể đặt máy {machine_name} ở vị trí trung tâm do chồng lấn!")

# Tính điểm ưu tiên cho từng cặp máy (dựa trên closeness và flow)
machine_pairs = []
for (m1, m2), closeness in closeness_dict.items():
    if m1 == m2 or closeness == '-':  # Bỏ qua trường hợp máy so sánh với chính nó hoặc closeness là '-'
        continue
    if closeness == 'U':
        continue  # Bỏ qua các cặp không có quan hệ đặc biệt
    flow = flow_dict.get((m1, m2), 0)
    score = closeness_weights[closeness] * flow  # Kết hợp closeness và flow
...
# Đặt các máy còn lại dựa trên mối quan hệ
for m1, m2, _ in machine_pairs:
    # Đặt m1 nếu chưa được đặt
    for i in range(machine_counts[m1]):
        ...
        # Thử từng vị trí trên mặt bằng (theo đơn vị mét, bước nhảy 0.1m)
        for y in np.arange(0, 20 - length + 0.1, 0.1):
            for x in np.arange(0, 40 - wid + 0.1, 0.1):
                ...
                # Kiểm tra giao thoa với các vùng đã chiếm
                overlap = False
                for _, prev_x1, prev_y1, prev_x2, prev_y2 in occupied_regions:
                    if not (x_upper_right <= prev_x1 or x_lower_left >= prev_x2 or
                            y_upper_right <= prev_y1 or y_lower_left >= prev_y2):
                ...
                if not overlap:
                    # Chuyển sang đơn vị ô
                    x_cells = int(x * 10)
                    y_cells = int(y * 10)
                    region = grid[y_cells:y_cells+length_cells, x_cells:x_cells+wid_cells]
                    if region.size == wid_cells * length_cells and np.all(region == None):
                        # Tính chi phí (khoảng cách đến các máy đã đặt)
                        cost = 0
                        x_center = x + wid / 2
                        y_center = y + length / 2
                        for placed_machine, (px, py) in machine_positions.items():
                            ...
                            if base_placed == m2:
                                ...
                                cost += (flow + closeness_weights[closeness]) * dist
        ...
        if not placed:
            print(f"Không đủ diện tích để bố trí máy {m1_name}")
    ...
    for j in range(machine_counts[m2]):
        ...
        if not placed:
            print(f"Không đủ diện tích để bố trí máy {m2_name}")
...
for machine_name in remaining_machines.copy():
    ...
    if not placed:
        print(f"Không đủ diện tích để bố trí máy {machine_name}")
