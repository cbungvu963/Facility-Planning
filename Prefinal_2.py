...
# Thêm khoảng cách an toàn (buffer) giữa các máy
buffer = 0.2  # 0.2 m khoảng cách an toàn giữa các máy
                ...
                if region.size == wid_cells * length_cells and np.all(region == None):
                    ...
                    for placed_machine, (px, py) in machine_positions.items():
                        ...
                        if base_placed == m2:
                            ...
                            dist = np.sqrt((x - px)**2 + (y - py)**2)
                            cost += (flow * 1000 + closeness_weights[closeness]) * dist
                        # Thêm chi phí với SH vì SH-MC cũng quan trọng
                        if base_placed == 'SH':
                ...
                if region.size == wid_cells * length_cells and np.all(region == None):
                    ...
                    for placed_machine, (px, py) in machine_positions.items():
                        ...
                        if base_placed == m1:
                            ...
                            dist = np.sqrt((x - px)**2 + (y - py)**2)
                            cost += (flow * 1000 + closeness_weights[closeness]) * dist
                        # Thêm chi phí với GRS vì GRS-QC cũng quan trọng
                        if base_placed == 'GRS':
...
# Đặt cặp GRS và QC (ưu tiên cao do lưu lượng tăng mạnh)
