...
machine_counts["MC"] = 2
machine_counts["QC"] = 2                                #000000000000000000000000000000 # Giới hạn tối đa là 1 máy cho các máy khác
...
# Danh sách máy và số lượng
machine_counts = {
    'MAL': 3, 'SH': 1, 'MC': 2, 'QC': 2, 'GRS': 1, 'MI': 1, 'LA': 1, 'SA W': 1,
...
# Bố trí máy trên mặt bằng (40m x 20m)
...
# Chuyển closeness và flow thành từ điển
...
buffer = 0.2  # Khoảng cách an toàn giữa các máy
...
# Đặt cặp MAL và SH (ưu tiên cao nhất)
m1, m2, _ = machine_pairs[0]  # MAL và SH
for i in range(machine_counts[m1]):  # Đặt MAL, MAL_1, MAL_2
    ...
    # Nếu là MAL (máy đầu tiên), đặt ở vị trí cố định
    if m1_name == 'MAL':
            ...
            for x in np.arange(0, 40 - wid + 0.1, 0.1):
                ...
                if not overlap:
                    ...
                    if region.size == wid_cells * length_cells and np.all(region == None):
                        # Ưu tiên đặt MAL gần trung tâm, lệch bên phải
                        cost = abs(x - 20) + abs(y - 10)  # Đặt gần (20, 10)
                        if cost < best_cost:
                            best_cost = cost
                            best_pos = (x, y)

    # Nếu là MAL_1 hoặc MAL_2, đặt gần máy trước đó (MAL hoặc MAL_1)
    else:
        # Tìm máy trước đó
        prev_machine = f"{m1}_{i-1}" if i > 1 else m1
        if prev_machine not in machine_positions:
            print(f"Cảnh báo: Máy '{prev_machine}' chưa được đặt. Đang thử tìm vị trí mới cho '{m1_name}'...")
            # Nếu máy trước đó chưa được đặt, tìm vị trí mới không phụ thuộc vào máy trước
            for y in np.arange(0, 20 - length + 0.1, 0.1):
                for x in np.arange(0, 40 - wid + 0.1, 0.1):
                    x = round(x, 1)
                    y = round(y, 1)
                    ...
                    if not overlap:
                        ...
                        region = grid[y_cells:y_cells+length_cells, x_cells:x_cells+wid_cells]
                        if region.size == wid_cells * length_cells and np.all(region == None):
                            cost = abs(x - 20) + abs(y - 10)  # Đặt gần trung tâm nếu không phụ thuộc máy trước
                            if cost < best_cost:
                                best_cost = cost
                                best_pos = (x, y)
        else:
            # Máy trước đó đã được đặt, đặt máy hiện tại gần máy trước đó
            ...
            for y in np.arange(0, 20 - length + 0.1, 0.1):
                for x in np.arange(0, 40 - wid + 0.1, 0.1):
                    ...
                    if not overlap:
                        ...
                        region = grid[y_cells:y_cells+length_cells, x_cells:x_cells+wid_cells]
                        if region.size == wid_cells * length_cells and np.all(region == None):
                            # Đặt gần máy trước đó
                            dist_to_prev = np.sqrt((x - prev_x)**2 + (y - prev_y)**2)
                            cost = dist_to_prev
                            # Nếu máy trước đó ở bên phải, đặt máy hiện tại bên trái máy trước đó
                            if x < prev_x - (prev_wid / 2 + wid / 2 + buffer):
                                cost += 0  # Ưu tiên đặt bên trái máy trước
                            else:
                                cost += 10  # Phạt nếu không đặt được bên trái
                            if cost < best_cost:
                                best_cost = cost
                                best_pos = (x, y)

    # Đặt máy nếu tìm được vị trí
    if best_pos:
        x, y = best_pos
        x_cells = int(x * 10)
        y_cells = int(y * 10)
        ...
        print(f"Cảnh báo: Không thể đặt máy '{m1_name}' do không tìm được vị trí phù hợp!")
...
for j in range(machine_counts[m2]):
    ...
    if best_pos:
        ...
        print(f"Cảnh báo: Không thể đặt máy '{m2_name}' do không tìm được vị trí phù hợp!")

# Đặt cặp MC và QC (ưu tiên cao)
                ...
                if region.size == wid_cells * length_cells and np.all(region == None):
                    ...
                    for placed_machine, (px, py) in machine_positions.items():
                        ...
                        if base_placed == 'GRS':
                            ...
                            cost += (flow * 1000 + closeness_weights[closeness]) * dist
                        if base_placed == 'SA W':
                            flow = flow_dict.get((m2, 'SA W'), 0)
                            closeness = closeness_dict.get((m2, 'SA W'), 'U')
                            dist = np.sqrt((x - px)**2 + (y - py)**2)
                            cost += (flow * 1000 + closeness_weights[closeness]) * dist
                        if base_placed == 'MI':
                            flow = flow_dict.get((m2, 'MI'), 0)
                            closeness = closeness_dict.get((m2, 'MI'), 'U')
                            dist = np.sqrt((x - px)**2 + (y - py)**2)
                            cost += (flow * 1000 + closeness_weights[closeness]) * dist
